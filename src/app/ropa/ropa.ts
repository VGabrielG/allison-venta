  ...
  template: `
    ...
    <div class="filters-container" *ngIf="showFilters">
      <div class="search-box">
        ...
        <input type="text" [(ngModel)]="searchTerm" placeholder="Buscar calzado..." (input)="filterAndSort()">
      </div>
        ...
        <div class="product-card" *ngFor="let prod of filteredProducts; let i = index"
          ...
          <div class="image-scroller" [ngClass]="{'multi-image': prod.imageUrls.length > 1}">
            <div class="product-image-container" *ngFor="let url of prod.imageUrls; let first = first" 
                 [ngClass]="prod.orientation || 'portrait'" (click)="openImage(prod, url)">
    ...
    <div class="image-modal" *ngIf="editModalOpen" (click)="closeEditModal()">
      <div class="edit-modal-content" (click)="$event.stopPropagation()">
        ...
        <div class="form-group"><label>Título</label><input type="text" [(ngModel)]="editForm.title"></div>
        <div class="form-group"><label>Descripción</label><textarea [(ngModel)]="editForm.description" rows="3"></textarea></div>
        <div class="form-group"><label>Precio (CLP)</label><input type="number" [(ngModel)]="editForm.price"></div>
        ...
        <div class="form-group">
          <label>{{ editingProduct?.id ? 'Cambiar imágenes (reemplaza todas)' : 'Imágenes' }}</label>
    ...
    <div class="image-modal" *ngIf="isTotalVisible" (click)="hideTotalSum()">
      <div class="total-modal-content" (click)="$event.stopPropagation()">
        <h2 class="total-title">Total Seccion Ropa</h2>
    ...
    <div class="image-modal" *ngIf="deleteModalOpen" (click)="cancelDelete()">
      <div class="confirm-modal-content" (click)="$event.stopPropagation()">
        ...
        <p>Esta acción eliminará <strong>{{ productToDelete?.title }}</strong> de Firestore y Storage.</p>
        <div class="form-actions">
...
export class RopaComponent implements OnInit, OnDestroy {
  ...
  imageOrientation: 'square' | 'portrait' | 'landscape' = 'portrait';
  showFilters = true;
  ...
  editForm: { title: string, description: string, price: number, orientation: 'square' | 'portrait' | 'landscape', condition?: 'Nuevo' | 'Como Nuevo' | 'Usado' | null } = { 
    title: '', description: '', price: 0, orientation: 'portrait', condition: null
  };
  selectedFiles: File[] = [];
  ...
  ngOnInit() {
    this.unsubscribe = this.fb.listenProducts('ropa', (products) => {
  ...
  openAddModal() {
    ...
    this.editForm = { title: '', description: '', price: 0, orientation: 'portrait', condition: null };
    this.previews = [];
  ...
  async saveProduct() {
    ...
    if (!this.previews.length) { alert('Sube al menos una imagen'); return; }
    this.saving = true;
    try {
      ...
      if (filesToUpload.length) {
        const res = await this.fb.uploadImages(filesToUpload, 'ropa');
  ...
  async downloadPDF() {
    ...
    try {
      ...
      pdf.save('Venta_Hogar_Allinson_Moda.pdf');
